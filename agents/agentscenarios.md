...
> **Cập nhật 2026-07-15 (quyết định PO):** trước đây bot thu slot dần qua chat (5 bước dưới
> đây: hỏi môn → lớp → mục tiêu → 1 lượt gộp tuỳ chọn → search). **Nay bỏ hẳn slot-filling qua
> chat cho lượt find_tutor ĐẦU TIÊN của phiên** — mọi ý định tìm gia sư (dù câu đã đủ hay thiếu
> thông tin) đều **LUÔN mở lại Mini App form** thay vì tự hỏi từng câu, để PH điền 1 lần đầy đủ
> (môn, lớp, ngân sách, khu vực, mong muốn...) qua UI thay vì chat. 5 bước "Logic thu slot" bên
> dưới mô tả hành vi **CŨ**, nay chỉ còn áp dụng cho **lượt SAU KHI Mini App form đã submit**
> (xem "Đối chiếu" cuối mục — code: `app/services/agent.py::_handle_find_tutor`,
> `_is_mini_app_submission`).
...
**Luồng mới (fresh find_tutor, `shown_tutors` rỗng — chưa search lần nào trong phiên):**
- PH nói bất kỳ câu gì thể hiện ý định tìm gia sư (kể cả đã nêu đủ môn/lớp/mục tiêu ngay
  trong câu đó) → agent nói 1 câu dẫn ngắn rồi **mở Mini App form** (`reopen_mini_app=true`),
  KHÔNG tự hỏi/tự search qua chat nữa.
- **Ngoại lệ DUY NHẤT:** message là đúng trigger cố định do chính Mini App form submit gửi lên
  (`"Tìm gia sư giúp tôi ạ"` / `"Please find a tutor for me."` — xem
  `tutora-zalo-bot/src/bot/flows/mini-app-search.flow.ts::handleFormSubmit`) → BỎ QUA gate mở
  form, đi thẳng vào logic thu slot bên dưới rồi search (tránh vòng lặp submit → mở form lại →
  submit → ...).

**Logic thu slot (CHỈ áp dụng SAU KHI Mini App đã submit — trigger message ở trên; thứ tự hỏi,
mỗi slot hỏi tối đa 1 lượt bình thường):**
...
4. **1 lượt gộp tuỳ chọn** (theo user flow: Area · Gender · criteria khác): hỏi GỘP trong 1
   ...
   (trích được ở các lượt trước, hoặc form đã có sẵn) → BỎ QUA lượt này.
...
> ⚠️ **Gap đã biết**: nếu Mini App form KHÔNG thu `goal` (field tuỳ chọn phía form), bước 3
> vẫn hỏi qua chat NGAY SAU lượt submit. Nếu PH trả lời câu hỏi `goal` đó ở lượt kế tiếp,
> message đó KHÔNG phải trigger cố định → gate "luôn mở form" ở trên lại fires (vì
> `shown_tutors` vẫn rỗng, chưa search lần nào) thay vì nhận câu trả lời goal — PH bị đưa lại
> vào form thay vì được hỏi tiếp mượt mà. Cách né tạm: đảm bảo Mini App form LUÔN thu `goal`
> (bắt buộc, không để tuỳ chọn) để không bao giờ rơi vào bước 3 sau submit.
...
- ✅ **Gate "LUÔN mở Mini App form" (2026-07-15) ĐÃ CÓ trong code** —
  `app/services/agent.py::_handle_find_tutor`, dòng đầu hàm (trước cả gate 3 slot cũ):
  `if not allowed and not _is_mini_app_submission(message)`. `allowed` = `shown_tutors`
  không rỗng (đã search/gợi ý gia sư trong phiên); `_is_mini_app_submission` khớp đúng 2
  trigger message cố định (`"Tìm gia sư giúp tôi ạ"` / `"Please find a tutor for me."`) mà
  `mini-app-search.flow.ts::handleFormSubmit` gửi lên. Test:
  `tests/agent/test_search_flow.py` (5 test, viết lại 2026-07-15 để khớp thiết kế mới).
- `app/services/agent.py::_handle_find_tutor` (phần logic thu slot CŨ, sau gate ở trên) vẫn
  đúng gate 3 slot (subject/grade/goal), KHÔNG chặn theo `preferences` — khớp quyết định chốt
  ở trên, nhưng CHỈ còn chạy tới được cho lượt PH submit Mini App form (bypass gate mới).
- ✅ **Lượt hỏi gộp tuỳ chọn (bước 4) ĐÃ CÓ trong code** (gate "hỏi 1 lần, mềm"): đủ 3 slot
  ...
  PH giục (rush) → bỏ qua cả câu hỏi goal lẫn lượt gộp, search ngay. ⚠️ Từ 2026-07-15, lượt
  "sau" đó CHỈ nhận được nếu vẫn đi qua gate mini-app-submission — PH trả lời câu hỏi gộp qua
  chat bình thường (không phải trigger message) sẽ bị gate mới đưa lại vào Mini App form thay
  vì được xử lý làm câu trả lời (xem gap ở mục KB-A phía trên).
...
- ✅ `tests/agent/test_search_flow.py` viết lại hoàn toàn 2026-07-15 cho khớp gate "LUÔN mở
  Mini App form": test trước đó kỳ vọng search thẳng qua chat khi đủ slot (KB-A bước 4 cũ) —
  nay agent KHÔNG còn tự search qua chat cho lượt find_tutor đầu tiên nữa, nên các test đó đã
  đổi assertion sang `reopen_mini_app is True, tutors == []`; thêm test riêng cho đúng 1 nhánh
  còn search thẳng (trigger message từ Mini App submit).
- ✅ **KB-A-2 bước 0 (hỏi rõ 2 trường hợp) ĐÃ CÓ trong code** —
  ...
  câu hỏi bước 0 dễ bị extract đoán nhầm intent). Phân loại câu trả lời (1)/(2)
  (`_classify_reopen_choice`): "1"/"2" map thẳng không qua LLM (tap numbered list), còn lại
  (trả lời tự do) qua LLM phân loại (`_classify_reopen_choice_llm`) — đã bỏ keyword cứng, bug
  thật 2026-07-17: "tôi muốn đổi nhu cầu" bị nhận nhầm "same" vì trùng từ khoá "muốn đổi"
  trong khi thiếu đúng cụm "nhu cầu khác"/"nhu cầu mới"; keyword cứng luôn vỡ với cách diễn
  đạt mới không lường trước, LLM tự suy luận ý PH ổn định hơn hẳn. Model tự trả "unclear"/lỗi
  → vẫn None, hỏi lại chứ không đoán bừa.
